...
export const getAllPizzas = ()=>async (dispatch) =>{
    ...
    try {
       ...
       // console.log(res);
        dispatch({type:'GET_PIZZAS_SUCCESS', paylod:res.data});
    } catch (error) {
        dispatch({type:'GET_PIZZAS_FAIL', paylod:error});  
    }
};
...
export const filterPizza = (searchkey, category) => async (dispatch) => {
  ...
  try {
    ...
    filterdPizza = response.data.filter((pizza) =>
      pizza.name.toLowerCase().includes(searchkey)
    );
    if (category !== "all") {
      filterdPizza = response.data.filter(
